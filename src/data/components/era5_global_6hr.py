...
class ERA5Dataset(Dataset):
    ...
    def __init__(self,
        ...
        diff_std = xr.open_dataset(f"{self.data_folder}/diff_{diff_stats}_1980_2019_6hr.nc", engine='netcdf4')
        self.diff_std = diff_std.sel(level=self.input_vars["levels"])
        raw_mean = xr.open_dataset(f"{self.data_folder}/raw_mean_1980_2019_6hr.nc", engine='netcdf4')
        self.raw_mean = raw_mean.sel(level=self.input_vars["levels"])
        raw_std = xr.open_dataset(f"{self.data_folder}/raw_std_1980_2019_6hr.nc", engine='netcdf4')
    ...
    def get_data(self, sample_time, left, right):
        ...
        for idx in range(-1 * left, right):
            ...
            # tp_data = []
            # for tp_acc_time in range(sample_start_time - 3600*(self.start_lead -1), sample_start_time+3600, 3600):
            #     yr_idx, t_idx = self.get_time_idx(tp_acc_time)
            #     tp_data.append(self.surf_data[yr_idx]['data'].sel(variable=['total_precipitation'])[t_idx].to_numpy())
            # tp_data = np.array(tp_data).sum(axis=0)
                
            yr_idx, t_idx = self.get_time_idx(sample_start_time)
            high_data = self.high_data[yr_idx]['data'][t_idx].to_numpy()
            # surf_data = self.surf_data[yr_idx]['data'].sel(variable=self.param_sfc[0:4])[t_idx].to_numpy()
            ...
            # surf_data = np.concatenate((surf_data, tp_data, dewpoint_temp_data[None, :, :]), axis=0) # add dewpoint temperature to surface data
            ...
            high_data = high_data[:, self.input_var_idxs["level"]]
    ...
    def __getitem__(self, idx):
        ...
        input = self.get_data(sample_start_time, 0, 1)
        if self.input_step > 1:
            previous_start_time = sample_start_time - self.start_lead * 3600
            previous_input = self.get_data(previous_start_time, 0, 1)
        ...
        if self.input_step > 1:
            return sample_start_time, input, output, static, time_features, autoregressive_step, previous_input
        else:
    ...
    def normalize_diff(self, x, reverse=False, data_pack=True):
        ...
        for i in range(n_surf_vars):
            param_name = self.input_vars["surface"][i]    
            if self.diff_stats == 'std': 
                std = self.diff_std[param_name].values 
            elif self.diff_stats == 'avgmax':
                std = self.diff_std[param_name].values / 3
            else:
                print("unknown diff stats")
        ...
        for j in range(n_high_vars): 
            ...
            for l in range(n_levels):
                if self.diff_stats == 'std': 
                    std = (self.diff_std[param_name].values[l]) 
                elif self.diff_stats == 'avgmax':
