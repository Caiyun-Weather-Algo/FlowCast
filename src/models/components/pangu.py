...
class Pangu(Module):
    def __init__(
        ...
        self.patchrecovery2d = PatchRecovery2D(
            img_size, patch_size[1:], 2 * embed_dim, 6
        )
    ...
    def forward(self, x):
        ...
        """
        surface = x[:, :6, :, :]
        upper_air = x[:, 6:, :, :].reshape(x.shape[0], 5, 13, x.shape[2], x.shape[3])
        ...
        output_upper_air = self.patchrecovery3d(output_upper_air).reshape(B, 5 * 13, 181, 360)
        return torch.cat((output_surface, output_upper_air), dim=1)


if __name__ == '__main__':
    device = "cuda" if torch.cuda.is_available() else "cpu"

    net = Pangu(img_size=(181, 360),
                patch_size=(2, 4, 4),
                embed_dim=192,
                num_heads=(6, 12, 12, 6),
                window_size=(2, 6, 12),
    ...
    input = torch.randn(1, 71, 181, 360).to(device)
    output = net(input)
